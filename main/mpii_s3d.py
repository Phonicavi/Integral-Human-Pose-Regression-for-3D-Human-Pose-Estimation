    ...
    with torch.no_grad():
        for itr, input_img in enumerate(tqdm(tester.batch_generator)):
            ...
            if vis:
                ...
                os.makedirs(osp.join(cfg.vis_dir, '%d-mpii' % args.test_epoch), exist_ok=True)
                cv2.imwrite(osp.join(cfg.vis_dir, ('%d-mpii/' % args.test_epoch) + filename + '_output.jpg'), tmpimg)
