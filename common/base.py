...
class BaselineTrainer(Base):

    def __init__(self, cfg):
        self.JointLocationLoss = DataParallelCriterion(loss.JointLocationLoss())
        super(BaselineTrainer, self).__init__(cfg, log_name='train_logs.txt')
...
class BaselineTester(Base):

    def __init__(self, cfg, test_epoch):
        self.coord_out = loss.soft_argmax
        self.test_epoch = int(test_epoch)
        super(BaselineTester, self).__init__(cfg, log_name='test_logs.txt')
